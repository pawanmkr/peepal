...
export class TutorService {
	...
	async create(dto: CreateTutorDto, userId: UUID) {
		...
		return this.sequelize.transaction(async (t) => {
			...
			this.logger.debug(`Created ${tutor.formalEducation.length} formal education records for tutor with id: ${tutor.id}`);
	...
	async update(id: UUID, dto: UpdateTutorDto) {
		...
		await this.sequelize.transaction(async (transaction) => {
			...
			if (dto.formalEducation && dto.formalEducation.length > 0) {
				this.logger.debug(`Found ${dto.formalEducation.length} formal education records for tutor with id: ${tutor.id}`);
				...
				this.logger.debug(`Deleted all formal education records for tutor with id: ${tutor.id}`);
				tutor.formalEducation = await this.formalEducationModel.bulkCreate(
					dto.formalEducation.map((education) => ({
						id: uuidv7() as UUID,
						...education,
				...
				this.logger.debug(`Updated ${tutor.formalEducation.length} formal education records for tutor with id: ${tutor.id}`);
			}
			return tutor;
		});
